...
class IAMParagraphs(BaseDataModule):
    ...
    def __init__(self, args: argparse.Namespace = None):
        ...
        self.mapping.append(NEW_LINE_TOKEN)
    ...
    def prepare_data(self):
        ...
        for split in ['trainval', 'test']:
            ...
            properties.update({
                id_: {'crop_shape': crops[id_].size[::-1], 'label_length': len(label), 'num_lines': _num_lines(label)}
    ...
    def setup(self, stage: str = None):
        def _load_dataset(split: str, augment: bool) -> BaseDataset:
            ...
            return BaseDataset(X, Y, transform=get_transform(image_shape=self.dims[1:], augment=augment))
...
def validate_input_and_output_dimensions(input_dims: Optional[Tuple[int, ...]],
...
def get_paragraph_crops_and_labels(iam: IAM, split: str) -> Tuple[Dict[str, Image.Image], Dict[str, str]]:
    ...
    for form_filename in iam.form_filenames:
        ...
        para_bbox = [
            min([_['x1'] for _ in line_regions]), min([_['y1'] for _ in line_regions]),
            max([_['x2'] for _ in line_regions]), max([_['y2'] for _ in line_regions])
        ]
...
def get_transform(image_shape: Tuple[int, int], augment: bool) -> transforms.Compose:
    ...
    if augment:
        ...
        transforms_list = [
            transforms.CenterCrop(image_shape)    # pad image to image_shape with 0
        ]
...
def get_dataset_properties() -> dict:
    ...
    return {
        'label_length': {
            'min': min(_get_property_values('label_length')), 'max': max(_get_property_values('label_length'))
        },
        'num_lines': {
            'min': min(_get_property_values('num_lines')), 'max': max(_get_property_values('num_lines'))
        },
        'crop_shape': {
            'min': crop_shapes.min(axis=0), 'max': crop_shapes.max(axis=0)
        },
        'aspect_ratio': {
