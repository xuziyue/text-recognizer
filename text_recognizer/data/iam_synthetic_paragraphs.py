...
from text_recognizer.data.iam_paragraphs import IAMParagraphs, get_dataset_properties, resize_image, get_transform, \
    NEW_LINE_TOKEN, IMAGE_SCALE_FACTOR, TRAIN_FRAC
...
class IAMSyntheticParagraphs(IAMParagraphs):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == "fit" or stage is None:
            line_crops, line_labels = load_line_crops_and_labels('trainval', PROCESSED_DATA_DIRNAME)  # ~ 4 seconds for resized, 12 seconds for original line crops
            X, para_labels = generate_synthetic_paragraphs(line_crops=line_crops, line_labels=line_labels)  # < 1 second

            Y = convert_strings_to_labels(strings=para_labels, mapping=self.inverse_mapping, length=self.output_dims[0])  # ~ 3 seconds
            self.data_train = BaseDataset(X, Y, transform=get_transform(image_shape=self.dims[1:], augment=self.augment))
...
def generate_synthetic_paragraphs(line_crops: List[Image.Image],
                                  line_labels: List[str],
                                  max_batch_size: int = 9) -> Tuple[List[Image.Image], List[str]]:
    ...
    batched_indices_list.extend(generate_random_batches(values=indices, min_batch_size=2, max_batch_size=max_batch_size // 2))
    batched_indices_list.extend(generate_random_batches(values=indices, min_batch_size=2, max_batch_size=max_batch_size))
    batched_indices_list.extend(generate_random_batches(values=indices, min_batch_size=(max_batch_size // 2) + 1, max_batch_size=max_batch_size))
